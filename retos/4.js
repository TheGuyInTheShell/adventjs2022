...
determine si es posible empaquetar todas las cajas en una sola de manera que cada caja contenga a otra (que a su vez contenga a otra, y así sucesivamente).
...
fitsInOneBox = (boxes) => {
  return boxes
    .sort((a, b) => a.l - b.l)
    .map((element, index, copyArray) => {
      if (index === 0) return true;
      const previousElement = copyArray[index - 1];
      return Object.entries(element).every(
        ([key, value]) => value > previousElement[key]
      );
    })
