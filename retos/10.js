...
llegar al punto más alto y descender hasta el final. No puede volver a subir una vez que ha bajado, ni puede iniciar el viaje bajando. Veamos unos ejemplos:
...
checkJump = (heights) => {
  ...
  const right = heights.slice(0, indexValue);
  const left = heights.slice(indexValue + 1);
  const leftRes = left.slice(1).every((item, idx) => item <= left[idx]);
  const rightRes = right.slice(1).every((item, idx) => item >= right[idx]);
  return right.length > 0 && left.length > 0 && leftRes && rightRes;
};
