...
function howManyReindeers(reindeerTypes, gifts) {
  ...
  return gifts.map((e) => {
    let cm = e.weight;
    const w = reindeerTypes.filter((e) => e.weightCapacity < cm);
    let cc = w.reduce((a, c) => (a += c.weightCapacity), 0);
    const reindeers = w.map((re) => {
      let num = Math.floor(cm / cc);
      cc -= re.weightCapacity;
      cm -= num * re.weightCapacity;
      return { type: re.type, num };
    });
    return { country: e.country, reindeers };
  });
}
